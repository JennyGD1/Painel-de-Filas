...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderRowLivres(row, a) { const timestampUltimaChamada = ultimasChamadas[a.id]; let ultimaChamadaDisplay = '--:--:--'; let disponivelDisplay = '--:--:--'; if (timestampUltimaChamada) { const segundosDesdeUltima = Math.round((Date.now() - timestampUltimaChamada) / 1000); const duracaoFormatada = formatarDuracaoHHMMSS(segundosDesdeUltima); ultimaChamadaDisplay = duracaoFormatada; } row.innerHTML = `<td>${a.name}</td><td>${a.position || 'N/A'}</td><td>${formatarTempo(a.login_start)}</td><td>${ultimaChamadaDisplay}</td><td>${disponivelDisplay}</td>`; }
