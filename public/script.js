// script.js (VERSÃO FINAL ATUALIZADA)
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderRowLivres(row, a) { const timestampUltimaChamada = ultimasChamadas[a.id]; let ultimaChamadaDisplay = '--:--:--'; let disponivelDisplay = '--:--:--'; if (timestampUltimaChamada) { const segundosDesdeUltima = Math.round((Date.now() - timestampUltimaChamada) / 1000); const duracaoFormatada = formatarDuracaoHHMMSS(segundosDesdeUltima); ultimaChamadaDisplay = duracaoFormatada; disponivelDisplay = duracaoFormatada; } row.innerHTML = `<td>${a.name}</td><td>${a.position || 'N/A'}</td><td>${formatarTempo(a.login_start)}</td><td>${ultimaChamadaDisplay}</td><td>${disponivelDisplay}</td>`; }
    ...
    function reposicionarCardsDinamicamente() {
        ...
        // Remove qualquer estilo em linha que possa estar causando conflito
        ...
        // CASO 1: Apenas Chamadas Ativas oculto
        ...
        // CASO 2: Apenas Chamadas em Espera oculto
        ...
        // CASO 3: Ambos Espera e Ativas ocultos
        else if (esperaOculto && ativasOculto && !pausadosOculto) {
        ...
        // CASO 4: Chamadas Ativas + Operadores Pausados ocultos
        else if (!esperaOculto && ativasOculto && pausadosOculto) {
            cards['card-livres'].style.gridArea = 'ativas'; // 2 coluna 1 linha
            cards['card-indisponiveis'].style.gridArea = 'pausados'; // 3 coluna 1 linha
        }
        // CASO 5: Apenas Operadores Pausados oculto
        else if (!esperaOculto && !ativasOculto && pausadosOculto) {
            cards['card-livres'].style.gridArea = 'pausados';
            cards['card-indisponiveis'].style.gridArea = 'livres';
        }
        // CASO 6: Todos os cards da direita ocultos
            ...
            // Ajusta as colunas da esquerda para ocupar todo o espaço
    ...
    async function carregarAbandonoDia() {
        ...
        if (!grupoSelecionado) return;
        try {
            const response = await fetch(`/api/indicadores/abandono-dia?grupo=${encodeURIComponent(grupoSelecionado)}`);
    ...
    // NOVA FUNÇÃO 3: Carrega apenas a variação do abandono
    async function carregarVariacaoAbandono() {
        const grupoSelecionado = document.getElementById("select-grupo").value;
        if (!grupoSelecionado) return;
        try {
            const response = await fetch(`/api/indicadores/variacao?grupo=${encodeURIComponent(grupoSelecionado)}`);
    ...
    document.getElementById("select-grupo").addEventListener("change", () => {
        renderizarPainel();
        // Chama as três funções ao trocar de grupo para atualização imediata
        carregarTmaTme();
        carregarAbandonoDia();
        carregarVariacaoAbandono();
        buscarUltimasChamadas();
    ...
    // Observador de mutação para detectar mudanças de classe nos cards
    ...
    // Configura o observador para cada card
